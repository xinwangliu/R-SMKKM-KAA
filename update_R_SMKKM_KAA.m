function [Sigma,Hstar,CostNew] = update_R_SMKKM_KAA(KH,Sigma,GradNew,CostNew,Hstar,H0,lambda,numclass,option)
...
while costmax<costmin
    [costmax,Hstar] = cost_R_SMKKM_KAA(KH,stepmax,desc,SigmaNew,Hstar,H0,lambda,numclass);
...
while (stepmax-stepmin)>option.goldensearch_deltmax*(abs(deltmax)) && stepmax > eps
    ...
    [costmedr,Hstarr] = cost_R_SMKKM_KAA(KH,stepmedr,desc,SigmaNew,Hstar,H0,lambda,numclass);
    [costmedl,Hstarl] = cost_R_SMKKM_KAA(KH,stepmedl,desc,SigmaNew,Hstar,H0,lambda,numclass);     
